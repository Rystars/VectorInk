...
/* requires rsvp.js (included above)*/
...
class FileStorage {
	write(name, data) {
		...
		var promise = new RSVP.Promise(function(resolve, reject) {
			...
			var gotFileSystem = function(fileSystem) {
				if (name_arr.length > 1) {
					fileSystem.getDirectory(
						...
						{ create: true },
						gotDirectory,
						fail.bind(null, 'gotFileSystem - getDirectory')
					);
				} else {
					fileSystem.getFile(
						name,
						...
						gotFileEntry,
						fail.bind(null, 'gotFileSystem - getFile')
					);
				}
			};
			...
			var gotDirectory = function(directory) {
				...
				if (name_index == name_arr.length - 1) {
					directory.getFile(
						...
						{ create: true, exclusive: false },
						gotFileEntry,
						fail.bind(null, 'gotDirectory - getDirectory')
					);
				} else {
					directory.getDirectory(
						...
						{ create: true },
						gotDirectory,
						fail.bind(null, 'gotDirectory - getFile')
					);
				}
			};
			...
			window.resolveLocalFileSystemURL(
				...
				gotFileSystem,
				fail.bind(null, 'requestFileSystem')
			);
		});
		return promise;
	}
	read(name) {
		var promise = new RSVP.Promise(function(resolve, reject) {
			...
			window.resolveLocalFileSystemURL(
				...
				gotFileEntry,
				fail.bind(null, 'resolveLocalFileSystemURL')
			);
		});
		return promise;
	}
	...
	list(dir, fn) {
		window.resolveLocalFileSystemURL(
			cordova.file.dataDirectory + dir,
			function(fileSystem) {
