      ...
      return /*@__PURE__*/(function (Framework7Class) {
        ...
        Keypad.prototype.updateValue = function updateValue (newValue) {
          ...
          if (keypad.$inputEl && keypad.$inputEl.length > 0) {
			if(keypad.$inputEl.val() == 0){
				keypad.$inputEl.val('')
			}

            keypad.$inputEl.val(keypad.formatValue(keypad.value));
            keypad.$inputEl.trigger('change');
			keypad.$el.find('.toolbar .toolbar-inner .left').html(keypad.$inputEl.val())
			keypad.updatePlaceholder('')
          }
		};

        Keypad.prototype.clearValue = function clearValue () {
			var keypad = this;
			keypad.value = '';
			if(keypad.$inputEl){
				keypad.$inputEl.val('');
			...
			if(keypad.$el){
				keypad.$el.find('.toolbar .toolbar-inner .left').html('')
			}
        };

        Keypad.prototype.updatePlaceholder = function updatePlaceholder (newValue) {
		  var keypad = this;
		  if(keypad.$el){
			keypad.$el.find('.toolbar .toolbar-inner .left-pl').html(newValue)
		  }
        };
